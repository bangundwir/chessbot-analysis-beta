    ...
    <div className="space-y-4">
      ...
      <div className="game-card p-4">
        ...
        <div className="mt-3 text-sm" style={{ color: 'var(--text-light)' }}>
          Current Mode: <span className="font-medium text-white capitalize">{settings.mode.replace('-', ' ')}</span>
          {isAnalysisMode && <span className="ml-2 px-2 py-1 bg-blue-600 text-white text-xs rounded">ANALYSIS</span>}
        </div>
      </div>
      ...
      <div className="game-card p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <button onClick={onFlipBoard} className="chess-button secondary w-full">
            🔄 Flip Board
          </button>
          <button 
            onClick={onBotMove} 
      ...
      <div className="game-card p-4">
        ...
        <div className="flex items-center gap-3 mb-3">
          <input
            ...
            onChange={(e) => onSettingsChange({ analysisMode: e.target.checked })}
            className="chess-checkbox"
          />
          <label htmlFor="analysisMode" className="text-sm" style={{ color: 'var(--text-light)' }}>
            Enable Analysis Mode (Free play with arrows)
          </label>
        </div>
        
        {isAnalysisMode && (
          <div className="p-3 bg-blue-900/20 border border-blue-500/30 rounded text-sm" style={{ color: 'var(--text-light)' }}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs" style={{ color: 'var(--text-light)' }}>Fast (1)</span>
              <input
                type="range"
                min="1"
                max="15"
                value={settings.aiDepth}
                onChange={(e) => onSettingsChange({ aiDepth: parseInt(e.target.value) })}
                className="chess-slider flex-1"
              />
              <span className="text-xs" style={{ color: 'var(--text-light)' }}>Strong (15)</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              onChange={(e) => onSettingsChange({ showAnalysisArrows: e.target.checked })}
              className="chess-checkbox"
            />
            <label htmlFor="showAnalysisArrows" className="text-sm" style={{ color: 'var(--text-light)' }}>
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              onChange={(e) => onSettingsChange({ autoAnalysis: e.target.checked })}
              className="chess-checkbox"
            />
            <label htmlFor="autoAnalysis" className="text-sm" style={{ color: 'var(--text-light)' }}>
          ...
          <div className="analysis-panel">
            ...
            {bestMove && (
              <div className="text-sm mb-2" style={{ color: 'var(--text-light)' }}>
                Best Move: <span className="font-mono text-white bg-gray-700 px-2 py-1 rounded">{bestMove}</span>
              </div>
            )}
            {hintMove && (
              <div className="text-sm" style={{ color: 'var(--text-light)' }}>
                Hint: <span className="font-mono text-yellow-400 bg-gray-700 px-2 py-1 rounded">{hintMove}</span>
              </div>
            )}
          </div>
        )}
      ...
      <div className="game-card p-4">
        ...
        <div className="space-y-3">
          ...
          <button onClick={onCopyFen} className="chess-button secondary w-full text-sm">
            📋 Copy Current FEN
          </button>
        </div>
      </div>
