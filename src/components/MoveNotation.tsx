...
export function MoveNotation({ lastMove, moveNumber, currentTurn, className = '' }: MoveNotationProps) {
    ...
    <div className={`bg-gray-800 rounded-lg border border-gray-600 ${className}`}>
      <div className="p-3">
        ...
        <div className="mt-2 text-xs text-gray-400">
          Move #{Math.ceil(moveNumber)} • {currentTurn === 'w' ? 'White' : 'Black'} to move
        </div>
      </div>
    </div>
  );
