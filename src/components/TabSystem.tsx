...
export function TabSystem() {
  ...
  const closeTab = useCallback((tabId: string, forceClose = false) => {
  ...
  const closeAllTabs = useCallback(() => {
    const confirmed = window.confirm('Are you sure you want to close all tabs? This will create a new game.');
    if (confirmed) {
      setTabs([]);
      setTimeout(createNewTab, 0);
    }
  ...
  const closeOtherTabs = useCallback((keepTabId: string) => {
    const confirmed = window.confirm('Are you sure you want to close all other tabs?');
    if (confirmed) {
  ...
  return (
      ...
      {contextMenu && (
        ...
        >
          ...
          <button
            onClick={() => {
              const newName = prompt('Enter new tab name:', tabs.find(t => t.id === contextMenu.tabId)?.name);
