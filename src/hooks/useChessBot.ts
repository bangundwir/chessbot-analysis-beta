...
export const useChessBot = (
  ...
  const createAnalysisArrows = useCallback((bestMove: string | null) => {
  ...
  const handleAnalyzePosition = useCallback(async () => {
    setIsThinking(true);
    try {
      const analysisResult = await stockfishApi.getAnalysis(chess.fen(), settings.aiDepth);
      console.log('Analysis result:', analysisResult); // Debug log
      
      setAnalysis(analysisResult);
      
      if (analysisResult?.bestmove) {
        const cleanMove = stockfishApi.extractMoveFromString(analysisResult.bestmove);
  ...
  const handleSquareClick = useCallback((square: Square) => {
    console.log('Square click handler:', square, 'Game over:', chess.isGameOver(), 'Analysis mode:', settings.analysisMode);
    
    if (chess.isGameOver() && !settings.analysisMode) return;
    if (!isPlayerTurn() && !settings.analysisMode) return;

    const piece = chess.get(square);
    console.log('Piece at square:', piece, 'Selected square:', selectedSquare);
    ...
    // If clicking on a piece (more permissive for mobile)
      ...
      const canSelectPiece = settings.analysisMode || piece.color === chess.turn();
  ...
  }, [selectedSquare, availableMoves, chess, settings, clearSelection, isPlayerTurn, makeMove]);
  ...
  const handlePieceDrop = useCallback((sourceSquare: Square, targetSquare: Square) => {
    ...
    console.log('Game state - Game over:', chess.isGameOver(), 'Analysis mode:', settings.analysisMode, 'Player turn:', isPlayerTurn());
    ...
    if (!settings.analysisMode && piece.color !== chess.turn()) {
      console.log('Wrong player piece - piece color:', piece.color, 'current turn:', chess.turn());
      return false;
    }
  ...
  const handleBotMove = useCallback(async () => {
    ...
    setIsThinking(true);
    try {
      const bestMove = await stockfishApi.getBestMove(chess.fen(), settings.aiDepth);
  ...
  }, [chess, moveHistory, settings.aiDepth, settings.autoAnalysis, settings.analysisMode, updateGameState, createAnalysisArrows]);
  ...
  useEffect(() => {
    ...
    if (settings.mode === 'ai-vs-ai' && !chess.isGameOver() && !isThinking && !settings.analysisMode) {
  ...
  useEffect(() => {
    ...
    if (settings.mode === 'human-vs-ai' && !chess.isGameOver() && !isThinking && !isPlayerTurn() && !settings.analysisMode) {
      timeoutId = window.setTimeout(() => {
        handleBotMove();
  ...
  }, [settings.mode, settings.analysisMode, chess, isThinking, isPlayerTurn, handleBotMove]);
  ...
  }, [settings.analysisMode, settings.autoAnalysis, chess.fen(), isThinking, handleAnalyzePosition]);
